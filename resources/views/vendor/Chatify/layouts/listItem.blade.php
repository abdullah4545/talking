...
@if($get == 'saved')
    <table class="messenger-list-item m-li-divider 
        @if('user_'.Auth::user()->id == $id && $id != "0") m-list-active 
...
<table class="messenger-list-item @if($user->id == $id && $id != "0") m-list-active @endif" data-contact="{{ $user->id }}">
    <tr data-action="0">
        ...
        <td style="position: relative">
            ...
            @endif
        <a href="{{url('/user_profile_'.$user->id)}}"><div class="avatar av-m" 
        style="background-image: url('{{ asset($user->profilepic->file) }}');">
        </div></a> 
        </td>
        {{-- center side --}}
        <td>
